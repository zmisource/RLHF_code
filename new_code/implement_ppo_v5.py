...
class CustomSymPOTrainer(Trainer):
    ...
    def compute_kl(self, log_ratio):
        """
        log_ratio = log(pi) - log(ref)
        """
        # 限制数值范围，避免溢出
        log_ratio = torch.clamp(log_ratio, -20, 20)
        # r = pi_ref / pi = exp(-log_ratio)
        r = torch.exp(-log_ratio)
        return r - 1 + log_ratio  # (ref/pi - 1) - log(ref/pi) 等价于 (ref/pi - 1) + log(pi/ref)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        with torch.no_grad():
            kl_term_chosen = self.beta_kl * log_ratio_chosen.detach()
            kl_term_rejected = self.beta_kl * log_ratio_rejected.detach()
            weight_chosen = 1 - f_X_Y2 - kl_term_chosen 
            weight_rejected = f_X_Y1 + kl_term_rejected

        log_ratio_chosen_safe = torch.clamp(log_ratio_chosen, -20, 20) 
        log_ratio_rejected_safe = torch.clamp(log_ratio_rejected, -20, 20)
        ratio_chosen = torch.exp(log_ratio_chosen_safe)
        ratio_rejected = torch.exp(log_ratio_rejected_safe)

        ratio_chosen_clipped =  torch.clamp(ratio_chosen, min=1, max=1+self.clip_eps_max)
        ratio_rejected_clipped =  torch.clamp(ratio_rejected, min=1 - self.clip_eps_min, max=1)
    
        # term_clipped = ratio_clipped * advantage
        # include KL penalty
        term_chosen_clipped = ratio_chosen_clipped * weight_chosen
        term_rejected_clipped = ratio_rejected_clipped * weight_rejected
        objective = term_chosen_clipped - term_rejected_clipped
        ...
        micro_logs = {
            ...
            "weight_chosen": weight_chosen.mean().item(),
            "weight_rejected": weight_rejected.mean().item(),
            "ratio_chosen_clipped": ratio_chosen_clipped.mean().item(),
            "ratio_rejected_clipped": ratio_rejected_clipped.mean().item(),
        }
