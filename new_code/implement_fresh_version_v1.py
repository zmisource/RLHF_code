...
class ResidualRLHFTrainer(Trainer):
    def __init__(self, ref_model=None, beta=0.5, clip_eps_min=0.9, clip_eps_max=9, *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # surr2_c = torch.clamp(ratio_c_per_token, 1.0 - self.clip_eps, 1.0 + self.clip_eps) * weight_c_per_token
        # term_c_per_token = torch.min(surr1_c, surr2_c)
        term_c_per_token = torch.clamp(ratio_c_per_token, 1.0 - self.clip_eps_min, 1.0 + self.clip_eps_max) * weight_c_per_token

        # surr1_r = ratio_r_per_token * weight_r_per_token
        # surr2_r = torch.clamp(ratio_r_per_token, 1.0 - self.clip_eps, 1.0 + self.clip_eps) * weight_r_per_token
        # term_r_per_token = torch.max(surr1_r, surr2_r)
        term_r_per_token = torch.clamp(ratio_r_per_token, 1.0 - self.clip_eps_min, 1.0 + self.clip_eps_max) * weight_r_per_token
        ...
        j_sym = term_c_sum - 10 * term_r_sum - self.beta * total_kl_penalty
        loss = -j_sym.mean()
...
def main():
    ...
    # 取前8个样本用于测试
    # danajiunanataset = dataset.select(range(min(8, len(dataset))))
