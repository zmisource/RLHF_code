...
class ResidualRLHFTrainer(Trainer):
    def __init__(self, ref_model=None, beta=0.05, clip_eps_min=0.6, clip_eps_max=2, *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # ---------------------------------------------------------------------
        # 核心改进：REC-NOIS (无重要性采样) + Mask 门控截断
        # ---------------------------------------------------------------------
        with torch.no_grad():
            # 计算 ratio，并直接将其限制在 [0.1, 10.0] 区间
            # 这样极端偏离的样本权重最高只能放大 10 倍，最低保留 0.1 倍
            ratio_c_per_token = torch.clamp(torch.exp(log_ratio_c_per_token), min=0.1, max=10.0)
            ratio_r_per_token = torch.clamp(torch.exp(log_ratio_r_per_token), min=0.1, max=10.0)

            # 门控 Mask 逻辑：
            # Chosen (正样本)：我们希望推高它的概率。如果 ratio 已经突破了上限（比如 > 1 + 2.0），
            # 门卫拦截 (Mask=0)，梯度归零，防止跑飞。否则放行 (Mask=1)。
            mask_c = (ratio_c_per_token <= 1.0 + self.clip_eps_max).float()

            # Rejected (负样本)：我们希望压低它的概率。如果 ratio 已经跌破了下限（比如 < 1 - 0.6），
            # 说明惩罚得足够了，门卫拦截 (Mask=0)。否则放行 (Mask=1)。
            mask_r = (ratio_r_per_token >= 1.0 - self.clip_eps_min).float()

        # 灵魂一步：Loss 挂载！
        # 直接用 pi_logp 去乘 Advantage(weight) 和 Mask。
        # 这样底层的 PyTorch 梯度就是纯粹的: Advantage * 1.0 * ∇log(pi)
        term_c_per_token = pi_logp_c_per_token * weight_c_per_token * mask_c * ratio_c_per_token
        term_r_per_token = pi_logp_r_per_token * weight_r_per_token * mask_r * ratio_r_per_token
        ...
        term_r_sum = (term_r_per_token * pi_mask_r).sum(dim=-1) / pi_len_r  # (batch,)
        ...
        # 全局油门：将 Advantage 博弈的规模放大 5.0 倍
        # scale_factor = 5.0 
        j_sym = (term_c_sum - term_r_sum) - self.beta * total_kl_penalty
        # j_sym = term_c_sum - term_r_sum
        
        # 最大化 j_sym 即最小化 -j_sym
        loss = -j_sym.mean()
        ...
        with torch.no_grad():
            ...
            micro_logs = {
                ...
                "ratio_c_mean": ratio_c_mean,
                "ratio_r_mean": ratio_r_mean,
                "ratio_c_median": ratio_c_median,
                "ratio_r_median": ratio_r_median,
                "ratio_c_max": ratio_c_max,
                "ratio_r_max": ratio_r_max,
                "ratio_c_min": ratio_c_min,
                "ratio_r_min": ratio_r_min,
...
def parse_args():
    ...
    parser.add_argument("--clip_eps_min", type=float, default=0.6, help="Min clip value.")
    parser.add_argument("--clip_eps_max", type=float, default=2, help="Max clip value.")
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        # max_steps=100, # 你用于测试的限制，正式跑全量数据时建议注释掉或改大
        bf16=True,
