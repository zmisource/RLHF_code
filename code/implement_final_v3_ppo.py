...
class CustomSymPOTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.cliprange_ratio = cliprange_ratio
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        chosen_loss1 = - ratio_y1 * (1 - f_y2)
        chosen_loss2 = - torch.clamp(ratio_y1, 1 - self.cliprange_ratio, 1 + self.cliprange_ratio) * (1 - f_y2)
        chosen_loss = torch.max(chosen_loss1, chosen_loss2)

        ratio_y2 = torch.exp(log_ratio_y2)
        rejected_loss1 = - ratio_y2 * f_y1
        rejected_loss2 = - torch.clamp(ratio_y2, 1 - self.cliprange_ratio, 1 + self.cliprange_ratio) * f_y1
        rejected_loss = torch.max(rejected_loss1, rejected_loss2)

        J_sym_objective = chosen_loss - rejected_loss
        ...
        logs = {
            ...
            "chosen_loss": chosen_loss.detach().mean().item(),
            "rejected_loss": rejected_loss.detach().mean().item(),
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="/train/output_model/llama3-8b-sympo-1e-6_0.1_ppo", help="Directory to save checkpoints and final model.")
    ...
    # --- SymPO 特定参数 ---
    parser.add_argument("--cliprange_ratio", type=float, default=0.2, help="KL divergence penalty coefficient.")
...
def main():
    ...
    trainer = CustomSymPOTrainer(
        ...
        cliprange_ratio=args.cliprange_ratio,
