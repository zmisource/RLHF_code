...
class CustomSymPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        log_probs_pi = Z * chosen_log_probs_pi / (chosen_lens + 1e-8) + (1 - Z) * rejected_log_probs_pi / (rejected_lens + 1e-8)

        log_ratio_safe = torch.clamp(log_ratio, -20, 20) 
        ratio_selected = torch.exp(log_ratio_safe)
        
        advantage = Z - f_selected
        
        clip_min = Z * 1.0 + (1 - Z) * (1 - self.clip_eps_min)
        clip_max = Z * (1 + self.clip_eps_max) + (1 - Z) * 1.0
        
        # term_raw = ratio_selected * advantage
        # include KL penalty
        term_raw = ratio_selected * (advantage - self.beta_kl * log_ratio_safe)

        ratio_clipped = torch.where(ratio_selected < clip_min, clip_min, ratio_selected)
        ratio_clipped = torch.where(ratio_clipped > clip_max, clip_max, ratio_clipped)
    
        # term_clipped = ratio_clipped * advantage
        # include KL penalty
        term_clipped = ratio_clipped * (advantage - self.beta_kl * log_ratio_safe)

        # objective = (advantage - self.beta_kl * log_ratio_safe).detach() * ratio_clipped
        objective = term_clipped.detach() * log_probs_pi
...
def parse_args():
    ...
    # --- SymPO 特定参数 ---
    parser.add_argument("--beta_kl", type=float, default=0.05, help="KL divergence penalty coefficient.")
